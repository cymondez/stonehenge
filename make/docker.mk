...
buildx-create: ## Create Buildx Builder
	@docker buildx ls | findstr /C:"$(BUILDX_BUILDER)" >NUL 2>&1 || docker buildx create --name=$(BUILDX_BUILDER) --platform linux/amd64,linux/arm64
	@docker buildx use $(BUILDX_BUILDER)

PHONY += buildx-destroy
buildx-destroy: ## Destroy Buildx Builder
	@docker buildx rm $(BUILDX_BUILDER) >NUL 2>&1 || ver >NUL
...
docker-build: ## Build Stonehenge Docker image locally
	...
	@docker buildx bake -f docker-bake.hcl --pull --progress plain --no-cache --load --set *.platform=linux/$(CURRENT_ARCH)
